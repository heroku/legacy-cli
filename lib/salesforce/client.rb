...
# A Ruby class to call the Salesforce REST API.  You might use this if you want to
# manage your Salesforce apps from within a Ruby program, such as Capistrano.
# 
# Example:
# 
#   require 'salesforce'
#   salesforce = Salesforce::Client.new('me@example.com', 'mypass')
#   salesforce.create('myapp')
#
class Salesforce::Client
  ...
  def list_stacks(app_name, options={})
    ...
    JSON.parse resource("/apps/#{app_name}/stack").get(
  ...
  # Run a rake command on the Salesforce app and return all output as
  # a string.
...
Console sessions require an open dyno to use for execution.
  ...
  def run_console_command(url, command, prefix=nil)
    ...
    if Salesforce::Command.parse_error_xml(e.http_body)
      Salesforce::Command.extract_error(e.http_body)
  ...
  def process(method, uri, extra_headers={}, payload=nil)
    headers  = salesforce_headers.merge(extra_headers)
  ...
  def extract_warning(response)
    ...
    if response.headers[:x_salesforce_warning] && @warning_callback
      warning = response.headers[:x_salesforce_warning]
      @displayed_warnings ||= {}
  ...
  def salesforce_headers   # :nodoc:
    {
      'X-Salesforce-API-Version' => '2',
