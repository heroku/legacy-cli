module Salesforce
  module Helpers
    ...
    def confirm_command(app = app)
      ...
      raise(Salesforce::Command::CommandFailed, "No app specified.\nRun this command from app folder or set it adding --app <app name>") unless app

      confirmed_app = extract_option('--confirm', false)
      if confirmed_app
        unless confirmed_app == app
          raise(Salesforce::Command::CommandFailed, "Confirmed app #{confirmed_app} did not match the selected app #{app}.")
        end
        return true
      else
        display ""
    ...
    def run_command(command, args=[])
      Salesforce::Command.run_internal(command, args)
    end
