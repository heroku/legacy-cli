module Salesforce::Command
  class Ssl < BaseWithApp
    def list
      salesforce.list_domains(app).each do |d|
        if cert = d[:cert]
    ...
    def add
      usage  = 'salesforce ssl:add <pem> <key>'
