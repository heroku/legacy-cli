...
module Salesforce::Command
  class Pgbackups < BaseWithApp
    ...
    Salesforce::Command::Help.group("pgbackups") do |group|
    ...
    def pgbackup_client
      ...
      abort(" !   Please add the pgbackups addon first via:\nsalesforce addons:add pgbackups") unless pgbackups_url
    ...
    def index
      ...
      if backups.empty?
        display("No backups. Capture one with `salesforce pgbackups:capture`.")
      else
