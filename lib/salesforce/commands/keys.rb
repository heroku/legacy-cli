module Salesforce::Command
  class Keys < Base
    def list
      ...
      keys = salesforce.keys
      if keys.empty?
        display "No keys for #{salesforce.user}"
      else
        display "=== #{keys.size} key#{'s' if keys.size > 1} for #{salesforce.user}"
        keys.each do |key|
    ...
    def add
      ...
      salesforce.add_key(key)
    end

    def remove
      salesforce.remove_key(args.first)
      display "Key #{args.first} removed."
    end

    def clear
      salesforce.remove_all_keys
