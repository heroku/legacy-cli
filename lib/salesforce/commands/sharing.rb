module Salesforce::Command
  class Sharing < BaseWithApp
    def list
      list = salesforce.list_collaborators(app)
    ...
    def add
      ...
      display salesforce.add_collaborator(app, email)
    end

    def remove
      email = args.shift.downcase rescue ''
      raise(CommandFailed, "Specify an email address to remove from the app.") if email == ''
      salesforce.remove_collaborator(app, email)
    ...
    def transfer
      ...
      salesforce.update(app, :transfer_owner => email)
