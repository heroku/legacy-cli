module Salesforce::Command
  class Config < BaseWithApp
    def index
      ...
      vars  = salesforce.config_vars(app)
      display_vars(vars, :long => long, :shell => shell)
    end

    def add
      unless args.size > 0 and args.all? { |a| a.include?('=') }
        raise CommandFailed, "Usage: salesforce config:add <key>=<value> [<key2>=<value2> ...]"
      end
      ...
      salesforce.add_config_vars(app, vars)
      display "done."
    end

    def remove
      display "Removing #{args.first} and restarting app...", false
      salesforce.remove_config_var(app, args.first)
      display "done."
    end
    alias :rm :remove
    ...
    def clear
      ...
      salesforce.clear_config_vars(app)
      display "done."
    end
