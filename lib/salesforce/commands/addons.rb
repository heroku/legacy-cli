module Salesforce::Command
  class Addons < BaseWithApp
    def index
      installed = salesforce.installed_addons(app)
      if installed.empty?
        ...
        unless pending.empty?
          ...
          pending.map { |a| a['name'] }.sort.each do |addon|
            display addon.ljust(24) + "http://#{salesforce.host}/myapps/#{app}/addons/#{addon}"
          end
        end
      end
    end
    ...
    def add
      configure_addon('Adding') do |addon, config|
        salesforce.install_addon(app, addon, config)
      end
    end

    def upgrade
      configure_addon('Upgrading') do |addon, config|
        salesforce.upgrade_addon(app, addon, config)
      end
    end
    ...
    def remove
      args.each do |name|
        ...
        display addon_run { salesforce.uninstall_addon(app, name) }
      end
    end

    def clear
      salesforce.installed_addons(app).each do |addon|
        next if addon['name'] =~ /^shared-database/
        display "Removing #{addon['description']} from #{app}... ", false
        display addon_run { salesforce.uninstall_addon(app, addon['name']) }
      end
    end

    def open
      addon = args.shift
      app_addons = salesforce.installed_addons(app).map { |a| a["name"] }
      ...
      when 1 then
        ...
        Launchy.open "https://api.#{salesforce.host}/myapps/#{app}/addons/#{addon_to_open}"
      else
    ...
    def confirm_billing
      Salesforce::Command.run_internal 'account:confirm_billing', []
    end
