...
module Salesforce::Command
  class App < Base
    def login
      Salesforce::Command.run_internal "auth:reauthorize", args.dup
    end

    def logout
      Salesforce::Command.run_internal "auth:delete_credentials", args.dup
    ...
    def create
      remote  = extract_option('--remote', 'salesforce')
      ...
      name    = salesforce.create_request(name, {:stack => stack})
      display("Creating #{name}...", false)
      begin
        Timeout::timeout(timeout) do
          loop do
            break if salesforce.create_complete?(name)
            display(".", false)
            sleep 1
          end
        end
        ...
        addons.each do |addon|
          ...
          salesforce.install_addon(name, addon)
        end

        display app_urls(name)
      rescue Timeout::Error
        display "Timed Out! Check salesforce info for status updates."
      end

      create_git_remote(name, remote || "salesforce")
    end
    ...
    def rename
      ...
      salesforce.update(name, :name => newname)
      ...
      if remotes = git_remotes(Dir.pwd)
        remotes.each do |remote_name, remote_app|
          ...
          shell "git remote add #{remote_name} git@#{salesforce.host}:#{newname}.git"
    ...
    def info
      ...
      attrs = salesforce.info(name)

      attrs[:web_url] ||= "http://#{attrs[:name]}.#{salesforce.host}/"
      attrs[:git_url] ||= "git@#{salesforce.host}:#{attrs[:name]}.git"
    ...
    def rake
      ...
      cmd = args.join(' ')
      if cmd.length == 0
        display "Usage: salesforce rake <command>"
      else
        salesforce.start(app, "rake #{cmd}", :attached).each { |chunk| display(chunk, false) }
      end
    rescue Salesforce::Client::AppCrashed => e
    ...
    def console
      ...
      if cmd.empty?
        ...
        display salesforce.console(app, cmd)
      end
    ...
    def console_session(app)
      salesforce.console(app) do |console|
        console_history_read(app)

        display "Ruby console for #{app}.#{salesforce.host}"
    ...
    def dynos
      app = extract_app
      if dynos = args.shift
        current = salesforce.set_dynos(app, dynos)
        display "#{app} now running #{quantify("dyno", current)}"
      else
        info = salesforce.info(app)
    ...
    def workers
      ...
      if workers = args.shift
        current = salesforce.set_workers(app, workers)
    ...
    def destroy
      app = extract_app
      info = salesforce.info(app)
      url  = info[:domain_name] || "http://#{info[:name]}.#{salesforce.host}/"

      if confirm_command(app)
        redisplay "Destroying #{app} (including all add-ons)... "
        salesforce.destroy(app)
        if remotes = git_remotes(Dir.pwd)
          remotes.each do |remote_name, remote_app|
    ...
    protected
      ...
      def console_history_dir
        FileUtils.mkdir_p(path = "#{home_directory}/.salesforce/console_history")
        path
      end
      ...
      def create_git_remote(app, remote)
        ...
        shell "git remote add #{remote} git@#{salesforce.host}:#{app}.git"
