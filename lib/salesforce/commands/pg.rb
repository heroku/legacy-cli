require "salesforce/commands/help"
require "salesforce/pgutils"
require "salesforce-postgresql/client"

module Salesforce::Command
  class Pg < BaseWithApp
    include PgUtils

    Salesforce::Command::Help.group("salesforce-postgresql") do |group|
    ...
    def salesforce_postgresql_var_names
      pg_config_var_names.select { |n| n.match("SALESFORCE_POSTGRESQL") }
    end

    def config_vars
      @config_vars ||= salesforce.config_vars(app)
    end
    ...
    def extract_db(opts={})
      ...
      return name, database if opts[:include_shared] || name.match("SALESFORCE_POSTGRESQL")

      (name, database) = resolve_db_id(db_id, :default => salesforce_postgresql_var_names.first) # get any SALESFORCE_POSTGRESQL_*_URL next
      return name, database if name.match("SALESFORCE_POSTGRESQL")
    end

    def with_salesforce_postgresql_database
      if !salesforce_postgresql_var_names
    ...
    def salesforce_postgresql_client(url)
      uri = URI.parse(url)
      SalesforcePostgresql::Client.new(uri.user, uri.password, uri.path[1..-1])
    end

    def info
      (name, database) = extract_db(:include_shared => true)

      unless name.match("SALESFORCE_POSTGRESQL")
        attrs = salesforce.info(app)
      ...
      with_salesforce_postgresql_database do |name, url|
        database = salesforce_postgresql_client(url).get_database
        display("=== #{app} database #{name}")
        ...
        if @salesforce_postgresql_url
          display_info("URL", @salesforce_postgresql_url)
        end
    ...
    def promote
      ...
      abort(" !   Usage: salesforce pg:promote --db <DATABASE>") unless url
      ...
      salesforce.add_config_vars(app, {"DATABASE_URL" => url})
    ...
    def reset
      ...
      (name, url, primary) = resolve_db_id(db_id, :usage_message => " !   Usage: salesforce pg:reset --db <DATABASE>")
      ...
      if confirm_command
        ...
        if name == "SHARED_DATABASE_URL"
          salesforce.database_reset(app)
        else
          salesforce_postgresql_client(url).reset
        end
        display "done"
      end
    end

    # TODO: this should check that no salesforce database is pre-running
    def wait
      with_salesforce_postgresql_database do |name, url|
        ticking do |ticks|
          database = salesforce_postgresql_client(url).get_database
    ...
    def psql
      with_psql_binary do
        with_salesforce_postgresql_database do |name, url|
          ...
          salesforce_postgresql_client(url).ingress
          url = URI.parse(url)
    ...
    def backups
      ...
      backups = salesforce_postgresql_client.get_backups
    ...
    def with_optionally_named_backup
      ...
      backup = backup_name ? salesforce_postgresql_client.get_backup(backup_name) :
                             salesforce_postgresql_client.get_backup_recent
    ...
    def restore_with(restore_param)
      restore = salesforce_postgresql_client.create_restore(restore_param)
      restore_id = restore[:id]
      ticking do |ticks|
        restore = salesforce_postgresql_client.get_restore(restore_id)
