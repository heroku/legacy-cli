...
module Salesforce::Command
  Salesforce::Command::Help.group("Logging (Expanded)") do |group|
  ...
  class Logs < BaseWithApp
    def index
      ...
      salesforce.read_logs(app, options) do |chk|
    ...
    def cron
      display salesforce.cron_logs(app)
    end

    def drains
      if args.empty?
        puts salesforce.list_drains(app)
        return
      end
      ...
      case args.shift
        when "add"
          url = args.shift
          puts salesforce.add_drain(app, url)
          return
        when "remove"
          url = args.shift
          puts salesforce.remove_drain(app, url)
          return
        when "clear"
          puts salesforce.clear_drains(app)
          return
      end
      raise(CommandFailed, "usage: salesforce logs:drains <add | remove | clear>")
    end
