  ...
  describe Pg do
    before do
      ...
      @pg.stub!(:config_vars).and_return({
        ...
        "SALESFORCE_POSTGRESQL_RONIN_URL" => "postgres://database_url"
      })
      @pg.stub!(:args).and_return(["--db", "DATABASE_URL"])
      @pg.salesforce.stub!(:info).and_return({})
    end

    it "resets the app's database if user confirms" do
      @pg.stub!(:confirm_command).and_return(true)

      fake_client = mock("salesforce_postgresql_client")
      fake_client.should_receive("reset")

      @pg.should_receive(:salesforce_postgresql_client).with("postgres://database_url").and_return(fake_client)
    ...
    it "doesn't reset the app's database if the user doesn't confirm" do
      ...
      @pg.should_not_receive(:salesforce_postgresql_client)
      @pg.reset
    end
    ...
    context "promotion" do
      it "promotes the specified database" do
        ...
        @pg.salesforce.should_receive(:add_config_vars).with("myapp", {"DATABASE_URL" => @pg.config_vars["SHARED_DATABASE_URL"]})
      ...
      it "fails if no database is specified" do
        ...
        @pg.salesforce.should_not_receive(:add_config_vars)
        @pg.should_receive(:abort).with(" !   Usage: salesforce pg:promote --db <DATABASE>").and_raise(SystemExit)

        lambda { @pg.promote }.should raise_error SystemExit
      end

      it "does not repromote the current DATABASE_URL" do
        @pg.stub(:args).and_return(['--db', 'SALESFORCE_POSTGRESQL_RONIN_URL'])
        @pg.stub!(:confirm_command).and_return(true)

        @pg.salesforce.should_not_receive(:add_config_vars)
        @pg.should_receive(:abort).with(" !   DATABASE_URL is already set to SALESFORCE_POSTGRESQL_RONIN_URL.").and_raise(SystemExit)
    ...
    context "with_salesforce_postgresql_database" do
      it "exits if a non SALESFORCE_POSTGRESQL url is explicitly specified" do
        @pg.stub!(:config_vars).and_return({
          "DATABASE_URL"                => "postgres://shared",
          "SALESFORCE_POSTGRESQL_RONIN_URL" => "postgres://ronin",
          "SALESFORCE_POSTGRESQL_IKA_URL"   => "postgres://ika"
        })

        @pg.should_receive(:abort).with(" !  This command is only available for addon databases.").and_raise(SystemExit)
        lambda {
          @pg.with_salesforce_postgresql_database { |name, url| }
      ...
      it "defaults to the db DATABASE_URL references if no args" do
        ...
        @pg.with_salesforce_postgresql_database do |name, url|
          name.should == "SALESFORCE_POSTGRESQL_RONIN_URL"
        end
      end

      it "defaults to the first (alphabetical) SALESFORCE_POSTGRESQL_*_URL if no args and DATABASE_URL isn't helpful" do
        @pg.stub!(:args).and_return([])
        @pg.stub!(:config_vars).and_return({
          "DATABASE_URL"                => "postgres://shared",
          "SALESFORCE_POSTGRESQL_RONIN_URL" => "postgres://ronin",
        ...
        @pg.with_salesforce_postgresql_database do |name, url|
          name.should == "SALESFORCE_POSTGRESQL_IKA_URL"
        end
      end
    end
  end
