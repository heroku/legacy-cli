...
module Salesforce::Command
  describe Config do
    ...
    it "shows all configs" do
      @config.salesforce.should_receive(:config_vars).and_return({ 'A' => 'one', 'B' => 'two' })
    ...
    it "trims long values" do
      @config.salesforce.should_receive(:config_vars).and_return({ 'LONG' => 'A' * 60 })
    ...
    it "sets config vars" do
      ...
      @config.salesforce.should_receive(:add_config_vars).with('myapp', {'a'=>'1','b'=>'2'})
      @config.add
    end

    it "allows config vars with = in the value" do
      @config.stub!(:args).and_return(['a=b=c'])
      @config.salesforce.should_receive(:add_config_vars).with('myapp', {'a'=>'b=c'})
      @config.add
    end

    it "unsets config vars" do
      @config.stub!(:args).and_return(['a'])
      @config.salesforce.should_receive(:remove_config_var).with('myapp', 'a')
      @config.remove
    end

    it "resets config" do
      @config.should_receive(:deprecate)
      @config.salesforce.should_receive(:clear_config_vars).with('myapp')
      @config.clear
    end
  end
end
