...
module Salesforce::Command
  describe App do
    ...
    it "shows app info, converting bytes to kbs/mbs" do
      ...
      @cli.salesforce.should_receive(:info).with('myapp').and_return({ :name => 'myapp', :collaborators => [], :addons => [], :repo_size => 2*1024, :database_size => 5*1024*1024 })
    ...
    it "shows app info using the --app syntax" do
      ...
      @cli.salesforce.should_receive(:info).with('myapp').and_return({ :collaborators => [], :addons => []})
      @cli.info
    end
    ...
    it "creates without a name" do
      @cli.salesforce.should_receive(:create_request).with(nil, {:stack => nil}).and_return("untitled-123")
      @cli.salesforce.should_receive(:create_complete?).with("untitled-123").and_return(true)
      @cli.should_receive(:create_git_remote).with("untitled-123", "salesforce")
      @cli.create
    end

    it "creates with a name" do
      @cli.stub!(:args).and_return([ 'myapp' ])
      @cli.salesforce.should_receive(:create_request).with('myapp', {:stack => nil}).and_return("myapp")
      @cli.salesforce.should_receive(:create_complete?).with("myapp").and_return(true)
      @cli.should_receive(:create_git_remote).with("myapp", "salesforce")
      @cli.create
    end

    it "creates with addons" do
      @cli.stub!(:args).and_return([ 'addonapp', '--addons', 'foo:bar,fred:barney' ])
      @cli.salesforce.should_receive(:create_request).with('addonapp', {:stack => nil}).and_return("addonapp")
      @cli.salesforce.should_receive(:create_complete?).with("addonapp").and_return(true)
      @cli.salesforce.should_receive(:install_addon).with("addonapp", "foo:bar")
      @cli.salesforce.should_receive(:install_addon).with("addonapp", "fred:barney")
      @cli.should_receive(:create_git_remote).with("addonapp", "salesforce")
      @cli.create
    end

    it "creates with an alternate remote name" do
      @cli.stub!(:args).and_return([ 'alternate-remote', '--remote', 'alternate' ])
      @cli.salesforce.should_receive(:create_request).and_return("alternate-remote")
      @cli.salesforce.should_receive(:create_complete?).with("alternate-remote").and_return(true)
    ...
    it "renames an app" do
      ...
      @cli.salesforce.should_receive(:update).with('myapp', { :name => 'myapp2' })
      @cli.rename
    end

    it "runs a rake command on the app" do
      @cli.stub!(:args).and_return(([ 'db:migrate' ]))
      @cli.salesforce.should_receive(:start).
    ...
    it "runs a single console command on the app" do
      ...
      @cli.salesforce.should_receive(:console).with('myapp', '2+2')
      @cli.console
    end

    it "offers a console, opening and closing the session with the client" do
      @console = mock('salesforce console')
      @cli.stub!(:console_history_read)
      @cli.stub!(:console_history_add)
      @cli.salesforce.should_receive(:console).with('myapp').and_yield(@console)
      Readline.should_receive(:readline).and_return('exit')
      @cli.console
    end

    it "asks to restart servers" do
      @cli.salesforce.should_receive(:restart).with('myapp')
      @cli.restart
    end

    it "scales dynos" do
      @cli.stub!(:args).and_return(['+4'])
      @cli.salesforce.should_receive(:set_dynos).with('myapp', '+4').and_return(7)
      @cli.dynos
    end
    ...
    it "destroys the app specified with --app if user confirms" do
      ...
      @cli.salesforce.stub!(:info).and_return({})
      @cli.salesforce.should_receive(:destroy).with('myapp')
      @cli.destroy
    end
    ...
    it "doesn't destroy the app if the user doesn't confirms" do
      ...
      @cli.salesforce.stub!(:info).and_return({})
      @cli.salesforce.should_not_receive(:destroy)
      @cli.destroy
    end
    ...
    context "Git Integration" do
      ...
      it "creates adding salesforce to git remote" do
        with_blank_git_repository do
          @cli.salesforce.should_receive(:create_request).and_return('myapp')
          @cli.salesforce.should_receive(:create_complete?).with('myapp').and_return(true)
          @cli.create
          bash("git remote").strip.should == 'salesforce'
        end
      end
      ...
      it "doesn't add a git remote if it already exists" do
        with_blank_git_repository do
          @cli.salesforce.should_receive(:create_request).and_return('myapp')
          @cli.salesforce.should_receive(:create_complete?).with('myapp').and_return(true)
          bash "git remote add salesforce #{@git}"
          @cli.create
        end
      end

      it "renames updating the corresponding salesforce git remote" do
        with_blank_git_repository do
      ...
      it "destroys removing any remotes pointing to the app" do
        with_blank_git_repository do
          bash("git remote add salesforce git@heroku.com:myapp.git")
          @cli.stub!(:args).and_return(['--app', 'myapp'])
          @cli.stub!(:confirm_command).and_return(true)
          @cli.salesforce.stub!(:info).and_return({})
          @cli.salesforce.should_receive(:destroy)
          @cli.destroy
