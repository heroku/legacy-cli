...
module Salesforce
  describe Plugin do
    ...
    it "lives in ~/.salesforce/plugins" do
      Plugin.stub!(:home_directory).and_return('/home/user')
      Plugin.directory.should == '/home/user/.salesforce/plugins'
    end

    it "extracts the name from git urls" do
      Plugin.new('git://github.com/salesforce/plugin.git').name.should == 'plugin'
    end

    describe "management" do
      before(:each) do
        @sandbox = "/tmp/salesforce_plugins_spec_#{Process.pid}"
      ...
      it "installs pulling from the plugin url" do
        plugin_folder = "/tmp/salesforce_plugin"
        ...
        File.directory?("#{@sandbox}/salesforce_plugin").should be_true
        File.read("#{@sandbox}/salesforce_plugin/README").should == "test\n"
      end

      it "uninstalls removing the folder" do
        FileUtils.mkdir_p(@sandbox + '/plugin1')
        Plugin.new('git://github.com/salesforce/plugin1.git').uninstall
        Plugin.list.should == []
      end
      ...
      describe "deprecated plugins" do
        before(:each) do
          FileUtils.mkdir_p(@sandbox + '/salesforce-releases/lib')
        end

        after(:each) do
          FileUtils.rm_rf(@sandbox + '/salesforce-releases/lib')
        end

        it "should show confirmation to remove deprecated plugins" do
          Plugin.should_receive(:confirm).with("The plugin salesforce-releases has been deprecated. Would you like to remove it? (y/N)").and_return(true)
          Plugin.should_receive(:remove_plugin).with("salesforce-releases")
          Plugin.load!
        end
        ...
        it "should not prompt for deprecation if not in an interactive shell" do
          ...
          Plugin.should_not_receive(:remove_plugin).with("salesforce-releases")
          Plugin.load!
        end
      end
    end
